    ...
    public function valid_ip(?string $ip = null, ?string $which = null): bool
{
    ...
    // Tambahkan pengecekan apakah $which adalah null
    if ($which !== null) {
        ...
        $which = null; // Jika $which adalah null, setel ke null.
    }

    // Tetapkan opsi sebagai array kosong jika $which adalah null
    $options = ($which !== null) ? ['flags' => $which] : [];

    return (bool) filter_var($ip, FILTER_VALIDATE_IP, $options) || (!ctype_print($ip) && (bool) filter_var(inet_ntop($ip), FILTER_VALIDATE_IP, $options));
}
