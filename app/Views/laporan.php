                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="tel" id="noHp" class="form-control" disabled aria-label="Amount (to the nearest dollar)">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function laporanAntrian(tanggalMulai, tanggalSelesai) {
        ...
        var tabel = '<table id="tabelLaporan" class="display table table-striped table-hover" ><thead><tr><th>NO</th><th>NAMA</th><th>NO. HP</th><th>JUMLAH PESANAN</th><th>PEMBAYARAN</th><th>RINCIAN</th></tr></thead><tbody>'
        ...
        $.ajax({
            ...
            success: function(data) {
                for (let i = 0; i < data.length; i++) {
                    ...
                    tabel += '<td>' + (i + 1) + '</td>'
                    tabel += '<td>' + data[i].nama + '</td>'
                    tabel += '<td>' + data[i].noHp + '</td>'
                    tabel += '<td>' + data[i].jumlahPesan + '</td>'
                    tabel += '<td>' + formatRupiah(data[i].pembayaran.toString()) + '</td>'
                    tabel += "<td><button href='#' class='btn btn-inverse-success btn-sm' onClick='tampilkanRincian(" + data[i].id + ",\"" + data[i].nama + "\", \"" + data[i].noHp + "\")'><i class='mdi mdi-format-list-bulleted-type'></i><i class='mdi mdi-food-fork-drink'></i></button></td>"
                    tabel += '</tr>'
                }
                if (data.length == 0) {
                    tabel += "<td colspan='7' class='text-center'>Data Masih Kosong</td>"
    ...
    function tampilkanRincian(id, nama, noHp) {
        $("#nama").val(nama)
        $("#noHp").val(noHp)
        var isiPesanan = ""
        var totalHarga = 0
