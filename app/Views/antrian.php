        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="card-description">
                    Pelanggan yang memiliki pesanan belum dihidangkan.
                </p>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="number" id="noHp" class="form-control" disabled aria-label="Amount (to the nearest dollar)">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
            ...
            success: function(data) {
                if (data.length) {
                    for (let i = 0; i < data.length; i++) {
                        isiPesanan += "<tr><td>" + data[i].noHp + "</td><td>" + data[i].nama + "</td><td>"
                        ...
                        isiPesanan += "</label></td><td><button href='#' class='btn btn-inverse-warning btn-sm' onClick='modalRincian(" + data[i].id + ", \"" + data[i].nama + "\", " + data[i].noHp + "," + data[i].status + ")'><i class='mdi mdi-format-list-bulleted-type'></i><i class='mdi mdi-food-fork-drink'></i></button></td></tr>"
                    }
                } else {
    ...
    function tampilkanAntrianSelesai() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.length) {
                    for (let i = 0; i < data.length; i++) {
                        isiPesanan += "<tr><td>" + data[i].noHp + "</td><td>" + data[i].nama + "</td><td><label class='badge badge-success'>Selsai</label></td><td><button href='#' class='btn btn-inverse-success btn-sm' onClick='modalRincian(" + data[i].id + ", \"" + data[i].nama + "\", " + data[i].noHp + "," + data[i].status + ")'><i class='mdi mdi-playlist-check'></i><i class='mdi mdi-food'></i></button></td></tr>"
                    }
                } else {
    ...
    function modalRincian(id, nama, noHp, status) {
        $("#nama").val(nama)
        $("#noHp").val(noHp)
        $("#proses").show()
